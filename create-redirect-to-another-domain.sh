...
if [[ "$confirmation" =~ ^[Yy][Ee]?[Ss]?$ ]]; then
  ...
  wget -O redirect-to-another-domain-template.yml https://raw.githubusercontent.com/cbschuld/aws-cf-redirect-to-another-domain/main/redirect-to-another-domain.yml

  # Execute the AWS command with the user-provided variables
  aws cloudformation create-stack --stack-name "$stack_name" \
  --template-body file://redirect-to-another-domain-template.yml \
  --parameters \
  ...
  rm -f redirect-to-another-domain-template.yml
